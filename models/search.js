        ...
        sql.connect(config).then(function () {
            ...
            // new sql.Request().query('select Distinct([Seller Name]) from APTB').then(function (recordset) {
                ...
                // new sql.Request().query("select [Vendor Name],[Invoice Number],'LF' as portal from [dbo].[lf_view] union select [Vendor Name], [Invoice Number],'Sp' as portal from [dbo].[seller_portal_view1] union select [Vendor Name] as 'Vendor Name' ,[Invoice Number],'APTB' as portal from [dbo].[aptb_view1] union select  [Vendor Name], CAST([Invoice Number] AS varchar),'AP Payment' from [dbo].[ap_payment_view1]").then(function (recordset) {
                        new sql.Request().query("select * from [dbo].[search]").then(function (recordset) {
    ...
    .get((req, res) => {
        ...
        sql.connect(config).then(function () {
            ...
            new sql.Request().query(
                ...
                "select * from [dbo].[search] where [Vendor Name]='" + vendorid + "'"
                ...
                for (var i = 0; i < portalarray.length; i++) {
                    ...
                    if (portal == 'AP Payment') {
                        // console.log("appay");
                        new sql.Request().query("select * from [dbo].[view_ap_payment] where [Vendor Name]='" + vendorid + "'").then(function (appayrecordset) {
                    ...
                    if (portal == "APTB") {
                        // console.log("ap");
                        new sql.Request().query("select * from [dbo].[view_aptb] where [Vendor Name]='" + vendorid + "'").then(function (appayrecordset) {
                            if (appayrecordset.length > 0) {
                    ...
                    if (portal == "LF") {
                        new sql.Request().query("select * from [dbo].[view_lf] where [Vendor Name]='" + vendorid + "'").then(function (appayrecordset) {
                            if (appayrecordset.length > 0) {
                    ...
                    if (portal == "SP") {
                        new sql.Request().query("select * from [dbo].[view_seller_portal] where [Vendor Name]='" + vendorid + "'").then(function (appayrecordset) {
                            if (appayrecordset.length > 0) {
    ...
    .get((req, res) => {
        ...
        sql.connect(config).then(function () {
            ...
            //    console.log(recordset);   
                    ...
                    // Fetch data from AP Payment View
                    if (portal == 'AP Payment') {
                        new sql.Request().query("select * from [dbo].[view_ap_payment] where [Invoice Number]='" + queryid + "'").then(function (appayrecordset) {
                    ...
                    // Fetch data from APTB View
                    if (portal == "APTB") {
                        new sql.Request().query("select * from [dbo].[view_aptb] where [Invoice Number]='" + queryid + "'").then(function (appayrecordset) {
                            if (appayrecordset.length > 0) {
                                result['APTB'] = appayrecordset;
                    ...
                    // Fetch data from LF View
                    if (portal == "LF") {
                        new sql.Request().query("select * from [dbo].[view_lf] where [Invoice Number]='" + queryid + "'").then(function (appayrecordset) {
                            if (appayrecordset.length > 0) {
                                result['LF'] = appayrecordset;
                    ...
                    // Fetch data from SP View
                    if (portal == "SP") {
                        new sql.Request().query("select * from [dbo].[view_seller_portal] where [Invoice Number]='" + queryid + "'").then(function (appayrecordset) {
                            if (appayrecordset.length > 0) {
                                result['SP'] = appayrecordset;
                            }
                        });
